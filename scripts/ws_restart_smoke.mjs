...
const startedHost = await nextMsg(host, (m) => m.type === EVENT.GAME_STATE);
...
const roomReset = await nextMsg(host, (m) => m.type === EVENT.ROOM_STATE);
if (roomReset.payload.players?.some((p) => p.ready === true)) {
  throw new Error('restart should reset ready state for both players');
}

const restartedHost = await nextMsg(host, (m) => m.type === EVENT.GAME_STATE);
await nextMsg(guest, (m) => m.type === EVENT.GAME_STATE);
