...
const startedHost = await waitFor(host, (m) => m.type === EVENT.GAME_STATE);
await waitFor(guest, (m) => m.type === EVENT.GAME_STATE);

const firstMineCount = startedHost.payload.mineCount;

send(host, EVENT.GAME_RESTART, {});
const roomReset = await waitFor(host, (m) => m.type === EVENT.ROOM_STATE);
if (roomReset.payload.host?.ready !== false || roomReset.payload.guest?.ready !== false) {
  throw new Error('restart should reset ready state for both players');
}

const restartedHost = await waitFor(host, (m) => m.type === EVENT.GAME_STATE);
await waitFor(guest, (m) => m.type === EVENT.GAME_STATE);
if (restartedHost.payload.phase !== 'playing') {
  throw new Error('restart did not return to playing phase');
}
if (restartedHost.payload.lives !== 5) {
  throw new Error('restart did not reset lives to 5');
}
if (restartedHost.payload.mineCount !== firstMineCount) {
  throw new Error('mine count changed unexpectedly after restart');
}

host.close();
guest.close();
console.log(`ws_restart_smoke:ok room=${roomCode}`);
