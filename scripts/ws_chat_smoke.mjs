...
send(host, EVENT.CHAT_SEND, { text: 'hello-team' });
const hostChat = await nextMsg(host, (m) => m.type === EVENT.CHAT_MESSAGE);
const guestChat = await nextMsg(guest, (m) => m.type === EVENT.CHAT_MESSAGE);

if (hostChat.payload.text !== 'hello-team' || guestChat.payload.text !== 'hello-team') {
  throw new Error('chat text mismatch');
}
if (hostChat.payload.nickname !== 'Hoster' || guestChat.payload.nickname !== 'Hoster') {
  throw new Error('chat nickname mismatch');
}

host.close();
guest.close();
console.log(`ws_chat_smoke:ok room=${roomCode}`);
